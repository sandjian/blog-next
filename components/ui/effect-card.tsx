...
const Skeleton = () => {
    const scale = [1, 1.1, 1];
    const transform = ["translateY(0px)", "translateY(5px)", "translateY(0px)"];
    const sequence = [
      [
        ".circle-1",
        {
    ...
    return (
      <div className=" mt-10 relative flex items-center justify-center">
        ...
        <div className="absolute w-full flex justify-center">
          <motion.div
            className="h-32 w-px bg-gradient-to-b from-transparent via-yellow-500 to-transparent"
            animate={{ x: [-100, 100, -100] }} // Movimiento horizontal
            transition={{
              duration: 5,
              repeat: Infinity,
              repeatType: "loop",
    ...
    <div className="absolute inset-0">
      {[...Array(12)].map((_, i) => (
        <motion.span
          ...
          className="inline-block bg-yellow-300"
        ></motion.span>
      ))}
    </div>
  );
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "max-w-sm w-full mx-auto p-2 rounded-xl border border-[rgba(255,255,255,0.10)] dark:bg-[rgba(40,40,40,0.70)] bg-gray-100 shadow-[2px_4px_16px_0px_rgba(248,248,248,0.06)_inset] group",
        className
      )}
    >
      {children}
